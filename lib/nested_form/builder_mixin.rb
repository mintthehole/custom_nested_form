module NestedForm
  module BuilderMixin
    ...
    def fields_for_with_nested_attributes(association_name, *args)
      ...
      @fields ||= {}
      p options[:temp]
      if options[:temp].present?
        @fields[custom_fields_blueprint_id_for(association_name,options[:temp])] = { :block => block, :options => options }
      else
    ...
    def custom_fields_blueprint_id_for(association,id)
